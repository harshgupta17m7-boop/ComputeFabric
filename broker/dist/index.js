...
io.on('connection', (socket) => {
    ...
    socket.on('register_worker', (capabilities) => {
    ...
    socket.on('update_capabilities', (newCapabilities) => {
...
function recalculateScore(worker) {
...
app.post('/api/tasks/inference', async (req, res) => {
    ...
    try {
        ...
        const primaryResult = await new Promise((resolve, reject) => {
            ...
            const onResult = ({ taskId: returnedId, result, merkleRoot, error }) => {
        ...
        const verifierResult = await new Promise((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error("Verifier timed out")), 10000);
            const onResult = ({ taskId: returnedId, merkleRoot, error }) => {
                if (returnedId === taskId) {
                    clearTimeout(timeout);
